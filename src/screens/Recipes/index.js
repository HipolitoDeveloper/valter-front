...
import { List } from "../../components/RecipeList/style";
...
export default class Recipes extends Component {
  ...
  async loadRecipes() {
      try {
       ...
       this.setState({arrRecipes: arrRecipes})
        ...
        alert(`Não foi possível carregar as receitas ${JSON.stringify(error.message)}`)
      }
  }
  ...
  async loadCategoriesFilter() {
    try {
      ...
      alert(`Não foi possível carregar as categorias de receita ${JSON.stringify(error.message)}`)
    }
  }
    ...
    return  (
      <FilterContainer>
        <Carousel layout={'default'}
                  ...
                  onSnapToItem = { index => console.warn(`teste ${JSON.stringify(index)}`)}
                  sliderWidth={WIDTH}
                  itemWidth={70}
        ...
        return (
            <Container>
                <Header>
                    ...
                    <HeaderButtons onPress={()=> this.props.navigation.navigate('Recipe', {id:1})}>
                      <HeaderButton>
